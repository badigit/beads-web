...
/// Resolves the `bd` CLI binary path -- implementation lives in
/// `crate::config` alongside the rest of the process's configuration
/// resolution (Dolt host/port/user/password, credentials-file, PORT). Kept
/// re-exported here so `beads.rs`, `cli.rs`, `worktree.rs`, and `main.rs`
/// don't need to change their call sites.
pub use crate::config::{bd_command_path, find_bd};
...
mod tests {
    ...
    // `bd` CLI resolution tests (pick_executable, winget_bd_candidates) moved
    // to `crate::config::bd_cli_tests` along with the implementation.
